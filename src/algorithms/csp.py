...
def compute_csp(cities: [City], from_city: str, to_city: str, value_getter) -> ([City], float):
    ...
    index_path, cost = backtracking_search(node_matrix, [] + [get_city_id_by_name(cities, from_city)],
                                           get_city_id_by_name(cities, to_city),
                                           0)
    cities_path = []
...
def get_less_restrictive_value(node_matrix: [[CSPNode]], node: int) -> int:
    less_total_connections: float = float('inf')
    less_connections_index: int = -1
    possible_values = []
    node_matrix_len: int = len(node_matrix)
    for i in range(node_matrix_len):
        if node_matrix[node][i] != -1:
            possible_values.append(i)

    for i in possible_values:
        connections = 0
        for j in range(node_matrix_len):
...
def get_city_id_by_name(cities: [City], city_name: str) -> int:
